    ...
    <footer className="flex flex-row justify-between py-8 bg-soundscape-dark-grey text-soundscape-white
        ...
        <div className="2xl:flex xl:flex lg:hidden m:hidden sm:hidden xs:hidden"> 
            <For each={headerItems()}>
                {(element, i) => {
                    return (
                        <a className="flex h-10 w-auto text-soundscape-white text-center items-center text-base px-4 mx-3 cursor-pointer rounded-header-btn hover:bg-soundscape-dark-blue " href={element.link} key={i()}>
                            {element.name}
                        </a>
                    )
                }}
